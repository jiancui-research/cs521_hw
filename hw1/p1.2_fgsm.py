...
# Store the original input for distance calculation
original_x = x.clone().detach()

eps = 0.3
# Iterative FGSM approach
...
for i in range(0, 100):
    ...
    if current_x.grad is not None:
        current_x.grad.zero_()
    
    # Compute loss and gradient
    loss = L(N(current_x), torch.tensor([t], dtype=torch.long))
    loss.backward()
    
    # FGSM step
    adv_x = current_x - eps * current_x.grad.sign()

    # Project back to epsilon ball around original input
    # perturbation = torch.clamp(adv_x - original_x, -eps, eps)
    # adv_x = original_x + perturbation
    
    new_class = N(adv_x).argmax(dim=1).item()
    distance = torch.norm((original_x - adv_x), p=float('inf')).data
    print(f"epoch={i}, eps={eps:.6f} -> new_class={new_class}, distance={distance:.6f}")
    print(adv_x)
    
    if new_class == t:
        print("SUCCESS: Iterative FGSM attack worked!")
        break
    else: 
        # Update current_x for next iteration
        current_x = adv_x.clone().detach()
        current_x.requires_grad_()
